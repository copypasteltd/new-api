...
// 上游地址
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/cvai/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/cvai/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/cvai/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/cvai/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/cvai/models.json", base), fmt.Sprintf("%s/api/cvai/vendors.json", base)
}
